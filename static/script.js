...
function set_total() {
    ...
    for(let i = 1; i<table.rows.length; i++) {
        qty = Number(table.rows[i].cells[1].children[0].value);
        rate = Number(table.rows[i].cells[2].children[0].value);
...
function tax_applied() {
    ...
    for(let i = 0; i<table.rows.length; i++) {
        ...
        tax = 1 + (tax_rate/100);
        // document.getElementById('row_tax').rows[index].cells[6].children[0].value = Number(qty*rate*tax).toFixed(2)
        document.getElementById('est_table').rows[index].cells[4].children[0].value = Number(qty*rate*tax).toFixed(2);
        }
    }
}
...
function add_row() {
    let item_html = document.getElementById('table_item_list').outerHTML;
    let tax_html = document.getElementById('table_tax_list').outerHTML;
    let table = document.getElementById('est_table');
